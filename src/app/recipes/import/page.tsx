...
import { ArrowLeft, Camera, PenSquare } from "lucide-react";
...
export default function ImportRecipesPage() {
  return (
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-6 px-4 py-6 sm:px-6 lg:max-w-5xl lg:px-8 lg:py-10">
      <PageHeader
        title="Import Recipe"
        description="Search online recipes and import them to your recipe box."
        actions={
      ...
      <section className="grid gap-4 md:grid-cols-2">
        <Link className="rounded-2xl border bg-card p-4 shadow-subtle transition hover:-translate-y-0.5 hover:shadow-soft" href="/recipes/new">
        ...
        <Link className="rounded-2xl border bg-card p-4 shadow-subtle transition hover:-translate-y-0.5 hover:shadow-soft" href="/recipes/import/scan">
          <div className="mb-2 flex h-10 w-10 items-center justify-center rounded-xl bg-gravy-gold/10 text-gravy-brown">
            <Camera className="h-5 w-5" aria-hidden="true" />
          </div>
          <h2 className="text-lg font-semibold text-gravy-charcoal">Scan Recipe</h2>
